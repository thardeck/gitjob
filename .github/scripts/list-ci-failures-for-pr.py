...
def get_jobs_for_attempt(repo_owner, repo_name, run_data, attempt_number):
    jobs_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/actions/runs/{run_data['id']}"
    if attempt_number > 1:
        jobs_url += f"/attempts/{attempt_number}"
    jobs_url += "/jobs?filter=failure"

    jobs_response = make_github_request(jobs_url)
    if jobs_response.status_code != 200:
        print(f"Error fetching jobs for run {run_data['id']} attempt {attempt_number}: {jobs_response.status_code}")
        return None

    return jobs_response.json()

def process_single_attempt(repo_owner, repo_name, run, attempt_number):
    if attempt_number != run.get("run_attempt", 1):
        ...
        if attempt_response.status_code != 200:
            print(f"Error fetching attempt {attempt_number} for run {run['id']}: {attempt_response.status_code}")
            return []
    ...
    jobs_data = get_jobs_for_attempt(repo_owner, repo_name, run_data, attempt_number)
    if not jobs_data or not jobs_data.get("jobs", []):
        return []

    attempt_failures = []

    with ThreadPoolExecutor(max_workers=min(10, len(jobs_data["jobs"]))) as executor:
        futures = {
            executor.submit(process_job, repo_owner, repo_name, job, run_data, attempt_number): job
...
def get_failed_workflow_attempts(repo_owner, repo_name, pr_number):
    ...
    runs_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/actions/runs?head_sha={head_sha}&status=failure"
    ...
    with ThreadPoolExecutor(max_workers=min(10, len(all_attempts))) as executor:
        futures = {
            executor.submit(process_single_attempt, repo_owner, repo_name, run, attempt_number):
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                failed_attempts.extend(attempt_results)
            except Exception as exc:
...
def main():
    ...
    failed_attempts = get_failed_workflow_attempts(repo_owner, repo_name, pr_number)
